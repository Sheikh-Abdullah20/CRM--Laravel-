    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{ route('task.store') }}" method="POST" class="p-2">
                    @csrf

                    <div class="form-group">
                        <label for="task_owner">Task Owner</label>
                        <input type="text" class="form-control" name="task_owner" id="task_owner" placeholder="Task Owner" value="{{ auth()->user()->first_name . ' ' . auth()->user()->last_name }}">
                        @error('task_owner')
                    ...
                    <div class="form-group">
                        ...
                        <input type="datetime-local" class="form-control" name="due_date" id="due_date" value="{{ old('due_date') }}">
                        @error('due_date')
                            <span class="text-danger">
                                {{ $message }}
                            </span>
                    ...
                    <div class="form-group">
                        ...
                        <select  class="form-control" name="group" id="group">
                            ...
                            <option value="Leads" {{ old('group') === 'Leads' ? 'selected' : '' }} >Leads</option>
                            <option value="Accounts" {{ old('group') === 'Accounts' ? 'selected' : '' }} >Accounts</option>
                            <option value="Contacts" {{ old('group') === 'Contacts' ? 'selected' : '' }} >Contacts</option>
                        </select>
                        @error('group')
                            <span class="text-danger">
                                {{ $message }}
                    ...
                    <div class="form-group my-3" style="display:none" id="leads">
                        <select class="form-control" id="person_of_leads">
                            ...
                            <option value="{{ $lead->first_name . ' ' . $lead->last_name }}" {{ old('person_of_group') === $lead->first_name . ' ' . $lead->last_name ? 'selected' : '' }}>{{ $lead->first_name . ' ' . $lead->last_name }} 
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group my-3" style="display:none" id="contacts">
                        <select class="form-control" id="person_of_contacts">
                            ...
                            @foreach($contacts as $contact)
                                <option value="{{ $contact->contact_name }}" {{ old('person_of_group') === $contact->contact_name ? 'selected' : '' }} >{{ $contact->contact_name }}</option>
                            @endforeach
                        </select>
                        @error('person_of_group')
                            <span class="text-danger">
                    ...
                    <div class="form-group my-3" style="display:none" id="accounts">
                        <select class="form-control"  id="person_of_accounts">
                            ...
                            @foreach($accounts as $account)
                                <option value="{{ $account->account_name }}" {{ old('person_of_group') === $account->account_name ? 'selected' : '' }}>{{ $account->account_name }}</option>
                            @endforeach
                        </select>   
                        @error('person_of_group')
                            <span class="text-danger">
                    ...
                    <div class="form-group my-3">
                        ...
                        <input class="form-control" name="related_to" id="related_to" placeholder="Related To" value="{{ old('related_to') }}">  
                        @error('related_to')
                            <span class="text-danger">
                                {{ $message }}
                            </span>
                    ...
                    <div class="form-check mt-4 d-flex ">
                        ...
                        <label class="form-check-label mx-3">
                            <input class="form-check-input" type="checkbox" name="reminder" id="reminder" {{ old('reminder') ? 'checked' : '' }}>
                    ...
                    <div class="form-group my-4" style="display: none" id="reminder_timee">
                        <label for="descripiton">Reminder Time</label>
                        <input type="datetime-local" class="form-control" name="reminder_time" id="reminder_time" value="
                        {{ old('reminder_time') }}">
                    ...
                    <div class="form-group my-4 ">
                        ...
                        <textarea class="form-control" name="description" id="descripiton">{{ old('description') }}</textarea>
                    </div>



                    <div class="form-group">
                        <button class="btn btn-fill btn-primary" type="submit">Create Task</button>
                    </div>
